...
import { buildAbsoluteBase } from './utils/url.js';
...
export class SitemapGenerator {
  ...
  generate(posts) {
    const baseUrl = buildAbsoluteBase(this.config.site?.url, this.config.site?.baseUrl);
    const urls = [];
    ...
    for (const tag of tags) {
      urls.push(this.createUrl(
        `${baseUrl}/tag/${slugify(tag, { lower: true, strict: true })}/`,
        new Date(),
        'weekly',
        '0.7'
      ));
    ...
    for (const category of categories) {
      urls.push(this.createUrl(
        `${baseUrl}/category/${slugify(category, { lower: true, strict: true })}/`,
        new Date(),
        'weekly',
        '0.7'
      ));
